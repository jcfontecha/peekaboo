...
extension ToolResponse {
    ...
    public func toAgentToolResult() -> AnyAgentToolValue {
        ...
        if let firstContent = content.first {
            ...
            case let .resourceLink(uri, name, _, _, mimeType, _):
                let description = mimeType.map { "\(name) (\($0))" } ?? name
                return AnyAgentToolValue(string: "[Resource Link: \(description) @ \(uri)]")
    ...
    private func convertContentToAnyAgentToolValue(_ content: MCP.Tool.Content) -> AnyAgentToolValue {
        ...
        case let .resource(resource, annotations, _):
            var resourceDict: [String: AnyAgentToolValue] = [
                "type": AnyAgentToolValue(string: "resource"),
                "uri": AnyAgentToolValue(string: resource.uri),
                "mimeType": resource.mimeType.map { AnyAgentToolValue(string: $0) } ?? AnyAgentToolValue(null: ()),
            ]
            ...
            if annotations != nil {
                resourceDict["annotations"] = AnyAgentToolValue(string: "[annotations]")
            }
        ...
        case let .resourceLink(uri, name, title, description, mimeType, annotations):
            ...
            if annotations != nil {
                resourceLinkDict["annotations"] = AnyAgentToolValue(string: "[annotations]")
            }
